...
if ($CreateResources) {
    ...
    az webapp create -n $webAppName -g $resourceGroup -p $appServicePlanName --runtime "DOTNET|8.0"
...
Write-Host "Monitor logs: az webapp log tail -n $($resources['WebAppName']) -g $($resources['ResourceGroup'])" -ForegroundColor Gray
