...
# Build service names
# Use deterministic naming for storage account (not random) to ensure reusability
$storageAccount = ($ResourcePrefix -replace '-', '') + "stg" + $Location.ToLower()
...
# First check if our target storage account exists
if (Test-StorageAccountExists -StorageName $storageAccount -RgName $ResourceGroup) {
    ...
    # Check for any existing storage accounts from previous runs with random names
    Write-Host "Checking for existing storage accounts in resource group..." -ForegroundColor Gray
    $existingAccounts = az storage account list --resource-group $ResourceGroup --query "[?starts_with(name, 'filesvcstg')].name" --output tsv
    
    if ($existingAccounts) {
        $existingAccount = ($existingAccounts -split "`n")[0]  # Use the first existing account
        Write-Host "Found existing storage account '$existingAccount' from previous deployment" -ForegroundColor Yellow
        Write-Host "Reusing existing account instead of creating new one" -ForegroundColor Green
        $storageAccount = $existingAccount  # Update variable to use existing account
        $storageConnString = az storage account show-connection-string --name $storageAccount --resource-group $ResourceGroup --output tsv
    } else {
...
az rest --method PUT --uri $appSettingsUri --body $appSettingsBody --headers "Content-Type=application/json" | Out-Null
...
$buildOutput = dotnet publish src/FileService.Api/FileService.Api.csproj -c Release -o publish-staging --verbosity normal 2>&1
...
$criticalFiles = @("FileService.Api.dll", "appsettings.json")
foreach ($file in $criticalFiles) {
    if (-not (Test-Path "publish-staging\$file")) {
        Write-Error "Critical file missing from build: $file"
        exit 1
    }
}
...
$packageSize = (Get-Item "deploy-staging.zip").Length / 1MB
Write-Host "Deployment package created: $([math]::Round($packageSize, 2)) MB" -ForegroundColor Green

# Deploy to Azure
Write-Host "Deploying to Azure App Service..." -ForegroundColor Gray
$deployResult = az webapp deploy --resource-group $ResourceGroup --name $webAppName --src-path deploy-staging.zip --type zip
...
if ($LASTEXITCODE -ne 0) {
    ...
    # Test deployment
    try {
        $testUrl = "https://$webAppName.chinacloudsites.cn/api/control/status"
        Write-Host "Testing deployment at: $testUrl" -ForegroundColor Gray
        $response = Invoke-WebRequest -Uri $testUrl -Method GET -TimeoutSec 30
        if ($response.StatusCode -eq 200) {
            Write-Host "Deployment test successful - API is responding" -ForegroundColor Green
        }
    }
    catch {
        Write-Warning "Deployment test failed - API may still be starting up"
        Write-Host "Check application logs: az webapp log tail --name $webAppName --resource-group $ResourceGroup" -ForegroundColor Yellow
    }
}

# Step 11: Post-Deployment Diagnostics
Write-Host "Running post-deployment diagnostics..." -ForegroundColor Yellow

# Verify app settings
Write-Host "Verifying app settings configuration..." -ForegroundColor Gray
$criticalSettings = @("ASPNETCORE_ENVIRONMENT", "Persistence__UseTableStorage", "Persistence__UseEf", "TableStorage__ConnectionString")
$settingsCheck = az webapp config appsettings list --name $webAppName --resource-group $ResourceGroup --query "[?contains('$($criticalSettings -join '|')', name)].{name:name, value:value}" --output json | ConvertFrom-Json

foreach ($setting in $criticalSettings) {
    $found = $settingsCheck | Where-Object { $_.name -eq $setting }
    if ($found) {
        $displayValue = if ($found.value -like "@Microsoft.KeyVault*") { "Key Vault Reference" } else { $found.value }
        Write-Host "  âœ“ $($setting): $displayValue" -ForegroundColor Green
    } else {
        Write-Warning "  âœ— $($setting): NOT SET"
    }
}

# Check for common startup logs
Write-Host "Checking for recent startup logs..." -ForegroundColor Gray
try {
    # Download recent logs to check for startup messages
    $timestamp = (Get-Date).ToString("yyyyMMdd-HHmmss")
    az webapp log download --name $webAppName --resource-group $ResourceGroup --log-file "deployment-logs-$timestamp.zip" | Out-Null
    
    if (Test-Path "deployment-logs-$timestamp.zip") {
        Expand-Archive -Path "deployment-logs-$timestamp.zip" -DestinationPath "deployment-logs-$timestamp" -Force
        
        # Look for debug messages in recent logs
        $logFiles = Get-ChildItem "deployment-logs-$timestamp" -Recurse -File -Name "*.txt" | Sort-Object LastWriteTime -Descending | Select-Object -First 3
        
        foreach ($logFile in $logFiles) {
            $logContent = Get-Content "deployment-logs-$timestamp\$logFile" -ErrorAction SilentlyContinue
            $startupMessages = $logContent | Where-Object { $_ -like "*STARTUP DEBUG*" -or $_ -like "*Using Azure Table Storage*" -or $_ -like "*Using in-memory repository*" }
            
            if ($startupMessages) {
                Write-Host "Recent startup messages found:" -ForegroundColor Green
                $startupMessages | ForEach-Object { Write-Host "  $_" -ForegroundColor Cyan }
                break
            }
        }
        
        # Cleanup diagnostic logs
        Remove-Item -Recurse -Force "deployment-logs-$timestamp*" -ErrorAction SilentlyContinue
    }
}
catch {
    Write-Host "Could not retrieve startup logs for verification" -ForegroundColor Yellow
}
...
Write-Host "Storage Account: $storageAccount (reused across deployments)" -ForegroundColor Cyan
